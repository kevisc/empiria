...
npm run benchmark                     # engine throughput (ticks/s) for a typical patch and as node count scales
```
...
(large language models). This audit exists precisely so that correctness does
not depend on that fact. Every routine above is checked against an independent
authoritative reference (R, the `boot` package, the canonical MT19937 test
vector) to the stated tolerance, and the reference values are regenerated by
...
regardless of how the code was authored. Statistical method choices and the
interpretation of results are the author's; the AI tools accelerated
implementation and did not make statistical decisions.
